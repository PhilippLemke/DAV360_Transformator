...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        inFileTouren = "DAV-Trier_Eingabeformular_Touren.xlsx"
    ...
    for index, inFormRow in TourenFormIn.iterrows():
        ...
        sheetOut.cell(row=ri, column=ColumnsTouren["key"]).value = (
        ...
        sheetOut.cell(row=ri, column=ColumnsTouren["assignedGroups"]).value = (
            TakExcelTransformLib.Gruppen[gruppen].get("fullpath", "")
        )
        ...
        sheetOut.cell(row=ri, column=ColumnsTouren["bookingCode"]).value = inFormRow[
            "Lfd-Nr."
        ]
        sheetOut.cell(row=ri, column=ColumnsTouren["title"]).value = titel
        sheetOut.cell(row=ri, column=ColumnsTouren["category"]).value = (
            TakExcelTransformLib.Kategorie[kategorie]
        )
        sheetOut.cell(row=ri, column=ColumnsTouren["technique"]).value = (
            TakExcelTransformLib.Technik[inFormRow["Schwierigkeit (Technik)"]]
        )
        sheetOut.cell(row=ri, column=ColumnsTouren["stamina"]).value = (
        ...
        sheetOut.cell(row=ri, column=ColumnsTouren["Termine"]).value = (
            TakExcelTransformLib.getDates(date, enddate)
        )
        sheetOut.cell(row=ri, column=ColumnsTouren["datesAlternativeText"]).value = (
            "<p>&nbsp;</p>"
        )
        sheetOut.cell(row=ri, column=ColumnsTouren["leaders"]).value = (
            TakExcelTransformLib.getLeaders(inFormRow["Tourenleitung/Organisation"])
        )
        sheetOut.cell(row=ri, column=ColumnsTouren["destination"]).value = (
            TakExcelTransformLib.makeHTML(inFormRow["Gebirgsgruppe/Region/Ort"])
        )
        sheetOut.cell(row=ri, column=ColumnsTouren["season"]).value = (
            TakExcelTransformLib.Saison[Season]
        )
        sheetOut.cell(row=ri, column=ColumnsTouren["classification"]).value = (
            TakExcelTransformLib.Klassifizierung[inFormRow["Klassifizierung"]]
        )  # Achtung das ist im Formular verdreht
        # sheetOut.cell(row=ri, column=ColumnsTouren["characterisation"]).value = (
        #     TakExcelTransformLib.Eventart[inFormRow["Tourenart"]]
        # )  # Achtung das ist im Formular verdreht
        sheetOut.cell(row=ri, column=ColumnsTouren["requirements"]).value = (
            "<p>&nbsp;</p>"
        )
        sheetOut.cell(row=ri, column=ColumnsTouren["maxNumberOfParticipants"]).value = (
            TakExcelTransformLib.getMaxNumberOfParticipants(
                str(inFormRow["max. Zahl der Teilnehmenden"])
            )
        )
        sheetOut.cell(row=ri, column=ColumnsTouren["previewDiscussion"]).value = (
            TakExcelTransformLib.makeHTML(inFormRow["Vorbesprechung"])
        )
        sheetOut.cell(row=ri, column=ColumnsTouren["bookingState"]).value = ""
        sheetOut.cell(row=ri, column=ColumnsTouren["registerEnd"]).value = (
            TakExcelTransformLib.getDate(Anmeldeschluss)
        )
        sheetOut.cell(row=ri, column=ColumnsTouren["registration"]).value = (
            TakExcelTransformLib.makeHTML(inFormRow["Bemerkungen"])
        )
        sheetOut.cell(row=ri, column=ColumnsTouren["meetingPoint"]).value = (
            TakExcelTransformLib.makeHTML("")
        )
        Entfernung = str(inFormRow["Anfahrt km"])
        Ausgangsort = str(inFormRow["Ausgangsort"])
        sheetOut.cell(row=ri, column=ColumnsTouren["arrivalHints"]).value = (
            TakExcelTransformLib.makeHTML(
