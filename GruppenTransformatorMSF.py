...
if __name__ == "__main__":
    ...
    if useSeperateFiles:
        outFileTouren = os.path.join(
            outDir, "DAV_GruppenexportTouren_" + SeasonID + ".xlsx"
        )
        outFileEvents = os.path.join(
            outDir, "DAV_GruppenexportEvents_" + SeasonID + ".xlsx"
        )
    else:
        outFileTouren = os.path.join(
            outDir, "DAV_GruppenexportEventsTouren_" + SeasonID + ".xlsx"
        )
    ...
    ColumnsTouren = {
        ...
        "attachments": 24,
        "images": 25,
        "maxNumberOfParticipants": 26,
        "bookingState": 27,
        "prices": 28,
        "previewDiscussion": 29,
        "registerStart": 30,
        "registerEnd": 31,
        "registration": 32,
        "enquiryForm": 33,
    ...
    for index, inFormRow in GruppenFormIn.iterrows():
        ...
        if Kategorie == "Tour":
            ...
            sheetTouren.cell(row=riTour, column=ColumnsTouren["key"]).value = (
                TakExcelTransformLib.getKeyGroups(Titel, GrouppeShort, date)
            )
            sheetTouren.cell(
                row=riTour, column=ColumnsTouren["assignedGroups"]
            ).value = TakExcelTransformLib.GetGroup(Gruppe)["fullpath"]
            sheetTouren.cell(row=riTour, column=ColumnsTouren["bookingCode"]).value = (
                "T_"
                + GrouppeShort
                + "_"
                + str(ProgramYear)
            ...
            sheetTouren.cell(row=riTour, column=ColumnsTouren["category"]).value = (
                TakExcelTransformLib.getKategorieForGruppe(Gruppe)
            )
            sheetTouren.cell(row=riTour, column=ColumnsTouren["technique"]).value = (
                TakExcelTransformLib.Technik[inFormRow["Schwierigkeit"]]
            )
            sheetTouren.cell(row=riTour, column=ColumnsTouren["stamina"]).value = (
                TakExcelTransformLib.Ausdauer[inFormRow["Kondition"]]
            )
            sheetTouren.cell(row=riTour, column=ColumnsTouren["description"]).value = (
                TakExcelTransformLib.makeHTML(inFormRow["Beschreibung"])
            )
            sheetTouren.cell(row=riTour, column=ColumnsTouren["Termine"]).value = (
                TakExcelTransformLib.getDates(date, enddate)
            )
            sheetTouren.cell(
                row=riTour, column=ColumnsTouren["datesAlternativeText"]
            ).value = "<p>&nbsp;</p>"
            sheetTouren.cell(row=riTour, column=ColumnsTouren["leaders"]).value = (
                TakExcelTransformLib.getLeaders(inFormRow["Tourenleitung/Organisation"])
            )
            sheetTouren.cell(row=riTour, column=ColumnsTouren["destination"]).value = (
                TakExcelTransformLib.makeHTML(inFormRow["Gebirgsgruppe/Region/Ort"])
            )
            sheetTouren.cell(row=riTour, column=ColumnsTouren["season"]).value = (
                TakExcelTransformLib.Saison[Season]
            )
            sheetTouren.cell(
                row=riTour, column=ColumnsTouren["characteristic"]
            ).value = TakExcelTransformLib.Eventart[
                inFormRow["Klassifizierung"]
            ]  # Achtung das ist im Formular verdreht
            sheetTouren.cell(
                row=riTour, column=ColumnsTouren["classification"]
            ).value = TakExcelTransformLib.Klassifizierung["Gemeinschaftstour"]
            sheetTouren.cell(row=riTour, column=ColumnsTouren["requirements"]).value = (
                "<p>&nbsp;</p>"
            )
            sheetTouren.cell(
                row=riTour, column=ColumnsTouren["maxNumberOfParticipants"]
            ).value = TakExcelTransformLib.getMaxNumberOfParticipants(
                str(inFormRow["max. Zahl der Teilnehmenden"])
            )
            sheetTouren.cell(row=riTour, column=ColumnsTouren["bookingState"]).value = (
                ""
            )
            sheetTouren.cell(row=riTour, column=ColumnsTouren["registerEnd"]).value = (
                TakExcelTransformLib.getDate(Anmeldeschluss)
            )
            sheetTouren.cell(row=riTour, column=ColumnsTouren["meetingPoint"]).value = (
                TakExcelTransformLib.makeHTML("")
            )
            sheetTouren.cell(row=riTour, column=ColumnsTouren["arrivalHints"]).value = (
                TakExcelTransformLib.makeHTML(
                    "Ausgangsort: " + Ausgangsort + ", Entfernung: " + Entfernung + "km"
                )
            )
            sheetTouren.cell(
                row=riTour, column=ColumnsTouren["isPublicTransportAvailable"]
        ...
        if Kategorie == "Veranstaltung":
            ...
            sheetVeranstaltungen.cell(
                row=riEvent, column=ColumnsEvents["key"]
            ).value = TakExcelTransformLib.getKeyGroups(Titel, GrouppeShort, date)
            sheetVeranstaltungen.cell(
                row=riEvent, column=ColumnsEvents["bookingCode"]
            ).value = (
                "V_"
                + GrouppeShort
                + "_"
            ...
            sheetVeranstaltungen.cell(
                row=riEvent, column=ColumnsEvents["title"]
            ).value = Titel
            sheetVeranstaltungen.cell(
                row=riEvent, column=ColumnsEvents["subtitle"]
            ).value = ""
            sheetVeranstaltungen.cell(
                row=riEvent, column=ColumnsEvents["description"]
            ).value = TakExcelTransformLib.makeHTML(inFormRow["Beschreibung"])
            sheetVeranstaltungen.cell(
                row=riEvent, column=ColumnsEvents["Termine"]
            ).value = TakExcelTransformLib.getDates(date, enddate)
            sheetVeranstaltungen.cell(
                row=riEvent, column=ColumnsEvents["datesAlternativeText"]
            ).value = "<p>&nbsp;</p>"
            sheetVeranstaltungen.cell(
                row=riEvent, column=ColumnsEvents["assignedGroups"]
            ).value = TakExcelTransformLib.GetGroup(Gruppe)["fullpath"]
            sheetVeranstaltungen.cell(
                row=riEvent, column=ColumnsEvents["leaders"]
            ).value = TakExcelTransformLib.getLeaders(
                inFormRow["Tourenleitung/Organisation"]
            )
            sheetVeranstaltungen.cell(
                row=riEvent, column=ColumnsEvents["maxNumberOfParticipants"]
            ).value = TakExcelTransformLib.getMaxNumberOfParticipants(
                str(inFormRow["max. Zahl der Teilnehmenden"])
            )
            sheetVeranstaltungen.cell(
                row=riEvent, column=ColumnsEvents["registerEnd"]
            ).value = TakExcelTransformLib.getDate(Anmeldeschluss)
            sheetVeranstaltungen.cell(
                row=riEvent, column=ColumnsEvents["destination"]
            ).value = TakExcelTransformLib.makeHTML(
                inFormRow["Gebirgsgruppe/Region/Ort"]
            )
            riEvent = riEvent + 1
